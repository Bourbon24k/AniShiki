...
function preferredGenres(sources) {
  ...
  for (const [name, count] of tally) {
    ...
    groups
      .get(group)
      .push({ name, percentage: Math.round((count / total) * 100) });
  }
...
export async function getSiteProfile(userId) {
  ...
  const [lists, ratings, favorites, history, activity, collections, friends] =
    await Promise.all([
      supabase
        .from("lists")
        .select("*")
  ...
  const watchedMinutes = Math.round(
    historyRows.reduce((sum, r) => sum + (Number(r.seconds) || 0), 0) / 60,
  );
  ...
  return {
    ...
    byStatus: (status) =>
      listRows.filter((r) => r.status === status).map(toCard),
    ...
    history: historyRows.map((r) => {
      ...
      return {
        ...
        href:
          position > 0
            ? `/player/${r.release_id}?ep=${position}`
            : `/release/${r.release_id}`,
        badge:
          position > 0
            ? total
              ? `${position} серия из ${total}`
              : `${position} серия`
            : null,
        progress:
          position > 0 && total ? Math.min(100, (position / total) * 100) : 0,
      };
    }),
  };
}
